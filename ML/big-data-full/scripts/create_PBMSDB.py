...
#
# Creates a per-bond (PB) multiple-SOAP (MS) database for 
# all <ID_RUN>s in a given nominal composition. The code follows 
# the directory structure below (see on 05/10/2018-(10)).
# 
...
# usage:
# 
# python create_PBMSDB.py <CHEM_COMPOSITION>
#
# Note on 05/07/2019: do not forget to include LOB_NOT_FINISHED_BUT_ICOHP_OK
    ...
    if knf == "KNF: ": # ok! The LOBSTER and quippy clusters are compatible ...
        for ibnd in range(len(bonds_to_write)):
            ...
            if symbA+symbB == "ZrZr":
                ...
                stl = ALL_SOAPS_ZrZr # SOAPs to dump
            ...
            # to avoid repeated interactions ...
            # ... checking atom A ...
            if bond.get_symbA()[:2] == "Zr": sss = SS_SOAPS_Zr
            if bond.get_symbA()[:2] == "Cu": sss = SS_SOAPS_Cu
            if bond.get_symbA()[:2] == "Al": sss = SS_SOAPS_Al
            keysoap = id_run+"-"+str(sub_step)+"-"+bond.get_symbA()[2:]
            if not keysoap in stl.keys():
                stl[keysoap] = sss[bond.get_symbA()[2:]]
            # ... checking atom B ...
            if bond.get_symbB()[:2] == "Zr": sss = SS_SOAPS_Zr
            if bond.get_symbB()[:2] == "Cu": sss = SS_SOAPS_Cu
            if bond.get_symbB()[:2] == "Al": sss = SS_SOAPS_Al
            keysoap = id_run+"-"+str(sub_step)+"-"+bond.get_symbB()[2:]
            if not keysoap in stl.keys():
                stl[keysoap] = sss[bond.get_symbB()[2:]]
    else:
...
def write_files(element_symbol):
    ...
    fileobj = open(ssdir+"lobsterin-quippy-"+element_symbol)
    lines = fileobj.readlines()
    fileobj.close()
    lastA = -1
    # Setting up each one
...
DB_DIR = BD_DIR+"/"+CHEM_COMPOSITION+"-PBMSDB"
...
if os.path.exists(DB_DIR):
    sys.exit("The database directory <CHEM_COMPOSITION>-PBMSDB already exists, please rename it!")
else:
    os.mkdir(DB_DIR)
...
ALL_SOAPS_ZrAl = {}
ALL_SOAPS_CuAl = {}
SS_SOAPS_Zr = None # <SUB_STEP>s SOAPs for Zr
SS_SOAPS_Cu = None # <SUB_STEP>s SOAPs for Cu
SS_SOAPS_Al = None # <SUB_STEP>s SOAPs for Al
    ...
    if os.path.isdir(RUN_DIR):
        ...
        for sub_step in range(0, 15):
            if gsts[sub_step] == DONE: # or gsts[sub_step] == SCF_CONVERGED_LOB_NOT_FINISHED_BUT_ICOHP_OK:
                ftl = open(SOAPS_DIR+"/2000/"+str(sub_step)+"/SOAPS-Zr.vec","rb")
                SS_SOAPS_Zr = pkl.load(ftl)
                ftl.close()
                ftl = open(SOAPS_DIR+"/2000/"+str(sub_step)+"/SOAPS-Cu.vec","rb")
                SS_SOAPS_Cu = pkl.load(ftl)
                ftl.close()
                ftl = open(SOAPS_DIR+"/2000/"+str(sub_step)+"/SOAPS-Al.vec","rb")
                SS_SOAPS_Al = pkl.load(ftl)
                ftl.close()
                write_files("Cu")
                write_files("Zr")
                write_files("Al")
