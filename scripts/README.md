...
- 📄 [**G/ML/big-data-full/scripts/setup_ICOHP_jobs.py**](https://github.com/aryrfjr/PRM_4_113603/blob/main/scripts/G/ML/big-data-full/scripts/setup_ICOHP_jobs.py): this script reads the output 📥 [**zca-th300.dump**](https://github.com/aryrfjr/PRM_4_113603/blob/main/data_examples/G/ML/big-data-full/Zr49Cu49Al2/c/md/lammps/100/21/zca-th300.dump) above and:
   ...
   - Creates the directories from **0** to **14** for each **<SUB_RUN>** with:
     ...
     - 💾 [**Zr49Cu49Al2.scf.in**](https://github.com/aryrfjr/PRM_4_113603/blob/main/data_examples/G/ML/big-data-full/Zr49Cu49Al2/c/md/lammps/100/21/2000/0/Zr49Cu49Al2.scf.in): Quantum ESPRESSO input file for the ground state electronic structure simulation with DFT.
    
     - 💾 [**Zr49Cu49Al2-check.scf.in**](https://github.com/aryrfjr/PRM_4_113603/blob/main/data_examples/G/ML/big-data-full/Zr49Cu49Al2/c/md/lammps/100/21/2000/0/Zr49Cu49Al2-check.scf.in): same content of [**Zr49Cu49Al2.scf.in**](https://github.com/aryrfjr/PRM_4_113603/blob/main/data_examples/G/ML/big-data-full/Zr49Cu49Al2/c/md/lammps/100/21/2000/0/Zr49Cu49Al2.scf.in), but it was used just to test a specific input variable of Quantum ESPRESSO.
   ...
   - Uses quippy for **feature engineering** by computing the **feature vectors for the ML model** (SOAP descriptors) for each **<SUB_RUN>** and saves as:
...
- 📄 [**ETL_model/ML/big-data-full/scripts/create_SSDB.py**](https://github.com/aryrfjr/PRM_4_113603/blob/main/scripts/ETL_model/ML/big-data-full/scripts/create_SSDB.py): when executed in **per-bond** database mode (PB) for a given **\<NC>** it will:
  ...
  - write to a **per-bond** single SOAP database directory **ML/big-data-full/\<NC>-PBSSDB** files for all possible **bond-types SA-SB** (with **SA/SB** as Zr, Al, Cu):
    ...
    - 💾 [**DUB.info**](https://github.com/aryrfjr/PRM_4_113603/blob/main/data_examples/ETL_model/ML/big-data-full/Zr49Cu49Al2-PBSSDB/DUB.info): an information file with those bonds detected by LOBSTER and by quippy are not compatible. This output file header structure is composed by the columns **<RUN>**, **<SUB_RUN>**, and **bond**.
...
- 📄 [**ETL_model/ML/big-data-full/scripts/mix_SSDBs.py**](https://github.com/aryrfjr/PRM_4_113603/blob/main/scripts/ETL_model/ML/big-data-full/scripts/mix_SSDBs.py): this script creates mixed databases for a set of NCs with a specific number of bonds (the **training set size** in **Fig. 1** of the PRM paper) for each nominal composition. When executed the script will: ... per-bond type (two types of atoms SA-SB; with EA/EB as Zr, Al, Cu)  ... 

  - read from the per-bond single SOAP database directories (like Zr45Cu45Al10-PBSSDB):
 
    - 📥 [**SA-SB.bnd**](https://github.com/aryrfjr/PRM_4_113603/blob/main/data_examples/ETL_model/ML/big-data-full/Zr49Cu49Al2-PBSSDB/Al-Al.bnd) (with EA/EB as Zr, Al, Cu): to load the result of the labeling process, with bond distances and -ICOHP values (supervised labels) that were generated by the script create_SSDB.py.
   
    - 📥 [**SA-SB-SOAPS.vec**](https://github.com/aryrfjr/PRM_4_113603/blob/main/data_examples/ETL_model/ML/big-data-full/Zr49Cu49Al2-PBSSDB/Al-Al-SOAPS.vec) (with EA/EB as Zr, Al, Cu): to load the SOAP vectors for atoms in bonds (another feature input) that were generated by the script create_SSDB.py.
   
  - write the mixed database (for instance named as DB8):
 
    - 💾 [**DB8_SA-SB.bnd**](https://github.com/aryrfjr/PRM_4_113603/blob/main/data_examples/ETL_model/ML/big-data-full/DB8/DB8_Al-Al.bnd) (with EA/EB as Zr, Al, Cu): a file with the same structure of the per nominal composition file SA-SB.bnd (generated by the script create_SSDB.py) from which a set of lines are randomly selected for each nominal composition.
   
    - 💾 [**DB8_SA-SB.info**](https://github.com/aryrfjr/PRM_4_113603/blob/main/data_examples/ETL_model/ML/big-data-full/DB8/DB8_Al-Al.info) (with EA/EB as Zr, Al, Cu): an information file with the mixed database name, its bond type, and the number of bonds from each nominal composition that composes it.
   
    - 💾 [**DB8_SA-SB-SOAPS.vec**](https://github.com/aryrfjr/PRM_4_113603/blob/main/data_examples/ETL_model/ML/big-data-full/DB8/DB8_Al-Al-SOAPS.vec) (with EA/EB as Zr, Al, Cu): a file with the same structure of the per nominal composition file SA-SB-SOAPS.vec (generated by the script create_SSDB.py) from which a set of lines are randomly selected for each nominal composition.
