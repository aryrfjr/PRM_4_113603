...
while IFS=" " read -r ID_RUN SRATE N_THERMO N_RUN remainder
do
...
cp $BASE_DIR/$SYS_NAME/c/md/lammps/$ID_RUN_RESTART/zca-equ-nve.restart $RUN_DIR
cd $RUN_DIR
...
read_restart zca-equ-nve.restart
...
compute zrc zr msd average yes
compute cuc cu msd average yes
compute alc al msd average yes

fix zrf zr ave/time 1 1 1 c_zrc[1] c_zrc[2] c_zrc[3] c_zrc[4] file Zr.msd
fix cuf cu ave/time 1 1 1 c_cuc[1] c_cuc[2] c_cuc[3] c_cuc[4] file Cu.msd
fix alf al ave/time 1 1 1 c_alc[1] c_alc[2] c_alc[3] c_alc[4] file Al.msd
...
rm $RUN_DIR/zca-equ-nve.restart

done < $BASE_DIR/SRATES
